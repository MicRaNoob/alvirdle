 ...
 *  
 * Source JS game.js 
 * Main Code Alvirdle - Wordel with an Alvirne theme 
 * 
 * Revision 	Developer		Date 		Modifcation  
 * Rev 1.1 		A.S.			9/23/2022	Change the game name to Alvirdle
 * 
 *****************************************************************************/
...
function addLogo(){
	...
	let domName = 'Alvirdle'; // A.S.  Sep. 16 2022 Changed from 'Wordled' to 'Alvirdle'
...
function startMenu(){
	...
	menu.id = 'menu';
	let labels = ["Change number of letters", "Change difficulty"]
	for(i = 0; i < 2; i++){
		...
		menuBtn.innerText = (i == 0)? maxBlock + ' letters' :
		(
			(i == 1)? level : 
		(
			difficulty
		...
		menuOption.append(label)
		menuOption.append(menuBtn);
	...
	for(i = 3; i < 6; i++){
		...
		menuBtn.innerText = (i == 3)? 'high scores' :
		(
			(i == 4)? 'help' : 
		(
			'start game'
...
function gameStart(){
	...
	notification.innerText = 'Start guessing! Or click on Alvirdle at the top to return to start menu.'
	notification.className = 'notificationDiv'
...
function createLabeledDropdown(labelText, options) {
    // Create the div element
    var div = document.createElement("div");
    div.style.display = "flex";
    div.style.alignItems = "center";
    div.style.justifyContent = "space-between";
    div.style.marginBottom = "10px"; // Add some spacing between items

    // Create the label
    var label = document.createElement("label");
    label.innerHTML = labelText;
    label.style.marginRight = "10px"; // Space between label and dropdown
